...
function App(props) {
  ...
  const setResolution = (componentWidthProp, pageWidthProp) => { // function that controls resolution
    ...
    if (componentWidthProp !== oldResolutionWidth) {
      ...
      setPercentWidth(() => {
        if (pageWidthProp < 500) return 2000;
        if (pageWidthProp < 1200) return 1000;
        return 2000 / 3;
      });
      ...
      if (oldResolutionWidth !== undefined) { // if our app hasn't just mounted we change left, distance, 100% distance
        ...
        if (-changed > (20 - amountOfPages) * componentWidthProp) changed += componentWidthProp;
        setLeft(changed);
        setDistance(changed);
  ...
  const touchMove = (event) => { // this function runs on touch swipe
    ...
    if (currentDistance < 0 ? -(distance + currentDistance) <= carouselWidth * (19 - componentsOnPage + 1)
  ...
  return (
    <Container
      ...
      mouseEnd={mouseEnd}
    >
      <Content SHADESOFBLUE={appProps.SHADESOFBLUE} componentWidth={componentWidth} />
    </Container>
  );
}
